...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uFUM
...
static const int64 nTargetTimespan = 60 * 60; // One year
static const int64 nTargetSpacing = 1 * 6;
...
void ThreadScriptCheck() {
    RenameThread("friendscoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1397001481, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xd5a87fa9 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xd5, 0xa8, 0x7f, 0xa9 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FriendscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FriendscoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("FriendscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("friendscoin-miner");
    ...
    try { loop {
        ...
        printf("Running FriendscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FriendscoinMiner terminated\n");
        throw;
    }
}
