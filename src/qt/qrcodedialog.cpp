...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("FUM")
    updateDisplayUnit();
}
...
{
    QString ret = QString("friendscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FUM unit input in lnReqAmount, we generate the URI with FUM as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::FUM, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
